...
const Skill = () => {
  ...
  return (
    <section className='w-auto h-full float-right'>
      <div className='text-center justify-center flex-col'>
        <div className='w-auto h-full flex flex-col items-center justify-center'>
          <div className='w-auto'>
            <h1 className='text-red-400  font-ubuntu text-3xl py-5'>Skills</h1>
            <div className='h-full w-full p-10 bg-slate-100  rounded-md grid grid-cols-3 gap-5  overflow-hidden'>
              {skillrecord.map((skill, index) => (
                <div key={index} className='p-5 rounded-md bg-gradient-to-br from-slate-200 to-slate-300  shadow-sm shadow-slate-500 ' data-aos="fade-down"
                  ...
                  <div className='flex flex-wrap m-2 p-1'>
                    {skill.icons.map((icon, idx) => (
                      <Tooltip key={idx} message={icon.tooltip}> {/* Display icon name in tooltip */}
                        <div
                          className='group h-14 w-14 border-1 border-red-400 rounded-md  m-2 shadow-sm shadow-red-300 
